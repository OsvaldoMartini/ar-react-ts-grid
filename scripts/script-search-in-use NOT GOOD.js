...
(function (
  ...
  const collectElements = function collectElements(
    ...
    const wordsToFind = ["Documenti e estratti", "Crediti", "Portafoglio"];

    const foundWordsResult = findWordsInSomeText(collectionFound, wordsToFind);

    console.log("foundWordsResult", foundWordsResult);
    // Define the order
    ...
    function findWordsInSomeText(collection, wordsToFind) {
      const matchingItems = [];

      if (Array.isArray(collection)) {
        collection.forEach((item) => {
          if (
            item.elementIdentity &&
            item.elementIdentity.someText &&
            typeof item.elementIdentity.someText === "string"
          ) {
            wordsToFind.forEach((word) => {
              if (item.elementIdentity.someText.includes(word)) {
                matchingItems.push(item); // Push the entire item, not just the word info
                // If you want to avoid duplicates if multiple words match, you can break after the first match
                // return;
              }
            });
          }
