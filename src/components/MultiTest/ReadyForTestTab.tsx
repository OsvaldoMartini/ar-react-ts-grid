...
export class ReadyForTestTab extends React.Component<{ onClearAll?: () => void; onExecutionStart?: () => void }, RftState> {
  ...
  private doGenerateBash = async (saveCsvToo: boolean) => {
    ...
    const buildScript = (chunk: ResolvedCase[], chunkIdx: number, reportFile: string): string => {
      ...
      lines.push("# =============================================================");
      lines.push(`# CAPI Runner — ${isFlow ? "FLOW" : "INDEPENDENT"} Execution${isMultiChunk ? ` (Part ${chunkIdx + 1}/${chunks.length})` : ""}`);
      ...
      lines.push(`<title>CAPI Report${isMultiChunk ? ` · Part ${chunkIdx + 1}` : ""}</title>`);
      lines.push("<style>");
      ...
      lines.push(`<h1>🧪 CAPI Report${isMultiChunk ? ` <span style='color:#8b949e;font-size:14px'>Part ${chunkIdx + 1} / ${chunks.length}</span>` : ""}</h1>`);
