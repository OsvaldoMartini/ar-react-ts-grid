...
const AboutPanel: React.FC<Props> = ({ socketPort, sessionId }) => {
  ...
  return <main className={styles.page}>
    <header><div><button title="Back to dashboard" onClick={goBack}><ArrowLeft size={17}/></button><Info size={22}/><span><h1>{about?.product || 'AR Web'}</h1><p>Application information</p></span></div><button title="Refresh" onClick={refresh}><RefreshCw size={17}/></button></header>
