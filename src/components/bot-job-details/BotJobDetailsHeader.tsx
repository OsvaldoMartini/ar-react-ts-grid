...
const BotJobDetailsHeader: React.FC<BotJobDetailsHeaderProps> = ({
  ...
  const actions = useMemo<WorkspaceHeaderAction<BotJobWorkspaceAction>[]>(() => {
    ...
    return [
      { id: 'REFRESH', label: pendingAction === 'REFRESH' ? 'Refreshing…' : 'Refresh', disabled: pending || unavailable },
      { id: 'SHOW_BOT_JOB', label: 'Bot Job', tone: 'primary', active: activeSurface === 'botJob', disabled: pending || unavailable || activeSurface === 'botJob' },
      {
      ...
      { id: 'SHOW_PRE_SCAN', label: 'Pre Scan', tone: 'success', active: activeSurface === 'preScan', disabled: pending || unavailable || activeSurface === 'preScan' },
      { id: 'CLOSE', label: 'Close', tone: 'danger', disabled: false },
    ];
  }, [activeSurface, botJobId, connected, pendingAction]);
