...
test('submits edited typed parameters',()=>{
  ...
  render(<OCRConfigPanel data={data} onSelect={jest.fn()} onSave={onSave} onDelete={jest.fn()} onCleanup={jest.fn()} onClose={jest.fn()}/>);
...
test('selects profiles and protects the default from deletion',()=>{
  ...
  const {rerender}=render(<OCRConfigPanel data={data} onSelect={onSelect} onSave={jest.fn()} onDelete={jest.fn()} onCleanup={jest.fn()} onClose={jest.fn()}/>);
  fireEvent.change(screen.getByLabelText('Profile'),{target:{value:'1'}});
  expect(onSelect).toHaveBeenCalledWith(1);
  rerender(<OCRConfigPanel data={{...data,activeProfileId:1}} onSelect={onSelect} onSave={jest.fn()} onDelete={jest.fn()} onCleanup={jest.fn()} onClose={jest.fn()}/>);
  expect(screen.getByRole('button',{name:/delete/i})).toBeDisabled();
});

test('requires a profile name',()=>{
  const onSave=jest.fn();
  render(<OCRConfigPanel data={{...data,profiles:[{...data.profiles[1],name:''}],activeProfileId:2}} onSelect={jest.fn()} onSave={onSave} onDelete={jest.fn()} onCleanup={jest.fn()} onClose={jest.fn()}/>);
  fireEvent.click(screen.getByRole('button',{name:/^save$/i}));
