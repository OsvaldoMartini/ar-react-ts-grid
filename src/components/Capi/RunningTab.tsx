...
import { downloadExecution, ExecutionSummary, parseCSVToSummary } from "./reportGenerator";
...
interface RunningTabState {
  ...
  loadedExecution: ExecutionSummary | null;
}
...
export class RunningTab extends React.Component<{}, RunningTabState> {
  ...
  private loadCSV = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = ev => {
      ...
      if (summary) {
        this.setState({ loadedExecution: summary, pageIndex: 0 });
      } else {
        alert("Could not parse the CSV file. Make sure it was generated by CAPI.");
      }
    };
