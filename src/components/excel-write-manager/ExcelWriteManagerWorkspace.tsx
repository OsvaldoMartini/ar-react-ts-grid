...
const ExcelWriteManagerWorkspace: React.FC<Props> = ({
  state, busy, policyLocked, onPolicyChange, onCellChange, onSave,
}) => {
  ...
  const selected: ExcelWriteFileState | undefined = state.files.find(
    file => file.fileId === selectedId,
  );

  return (
    <section className={styles.workspace} aria-label="ExcelWriter files in React memory">
...
export default ExcelWriteManagerWorkspace;
