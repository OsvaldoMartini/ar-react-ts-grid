...
type Props = { data:OCRConfigData; busy?:boolean; error?:string; onSelect:(id:number)=>void; onSave:(draft:{profileId?:number;name:string;description:string;parameters:OCRParameter[];asNew:boolean})=>void; onDelete:(id:number)=>void; onClose:()=>void };

const OCRConfigPanel:React.FC<Props> = ({data,busy,error,onSelect,onSave,onDelete,onClose}) => {
  ...
  return <div className={styles.backdrop}><section className={styles.panel} aria-label="OCR configuration">
    ...
    <footer><button disabled={!active||active.default||busy} onClick={()=>active&&onDelete(active.id)}><Trash2 size={15}/>Delete</button><span/><button disabled={busy} onClick={()=>submit(true)}>Save as new</button><button className={styles.primary} disabled={busy} onClick={()=>submit(false)}><Save size={15}/>Save</button></footer>
  </section></div>;
};
